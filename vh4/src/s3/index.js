/**
 * @typedef {import('aws-sdk').S3.GetObjectOutput} getObjectResponse
 */
 ...
 */
exports.uploadObjectPromise = ({AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_BUCKET, AWS_REGION}, s3Options) => {
  const s3 = new S3({
    params: {
 ...
 *
 * @param {{AWS_ACCESS_KEY_ID:string,AWS_SECRET_ACCESS_KEY:string,AWS_BUCKET:string,AWS_REGION:string}} awsOptions - AWS information needed for initialization.
 * @param {{Key: string}} s3Options - S3 Key to be deleted.
 * @description getObjectPromise downloads an s3 file into a buffer
 * @returns {Promise<getObjectResponse>}  Returns the response data object of s3.
 * */
exports.deleteObjectPromise = ({AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_BUCKET, AWS_REGION}, s3Options) => {
  const s3 = new S3({
    params: {
      accessKeyId: AWS_ACCESS_KEY_ID,
      secretAccessKey: AWS_SECRET_ACCESS_KEY,
  ...
  return s3.deleteObject(s3Options).promise()
}
